...
import { UpdateVehicleEvaluationDetailsUseCase } from '@application/vehicle-evaluations/use-cases'

describe('UpdateVehicleEvaluationDetailsUseCase', () => {
  it('returns Success when exists and updated', async () => {
    const ve = { id: 've-1', updateDetails: jest.fn() }
    const repo = {
    ...
    const useCase = new UpdateVehicleEvaluationDetailsUseCase(repo as any)
