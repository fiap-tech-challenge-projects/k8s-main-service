...
import { ActivateEmployeeUseCase } from '@application/employees/use-cases'

describe('ActivateEmployeeUseCase', () => {
  it('returns Success when employee activated', async () => {
    const repo = {
      findById: jest.fn().mockResolvedValue({ id: 'e-1', activate: jest.fn() }),
      update: jest.fn().mockResolvedValue({ id: 'e-1', active: true }),
    }
    ...
    const useCase = new ActivateEmployeeUseCase(repo as any, userContextService as any)
